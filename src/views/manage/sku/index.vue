...
<div class="app-container">
  ...
  <el-row :gutter="10" class="mb8">
    ...
    <el-col :span="1.5">
      <el-button type="warning" plain icon="Upload" @click="handleImport" v-hasPermi="['manage:sku:add']">导入</el-button>
    </el-col>
  ...
  <el-dialog :title="title" v-model="open" width="500px" append-to-body>
    <el-form ref="skuRef" :model="form" :rules="rules" label-width="80px">
      ...
      <el-form-item label="商品价格" prop="price">
        <el-input-number v-model="form.price" :min="0.01" :max="999.99" :precision="2" :step="0.5"
          placeholder="请输入商品价格" />
      </el-form-item>
  ...
  <!-- 数据导入对话框 -->
  <el-dialog title="数据导入" v-model="importOpen" width="300px" append-to-body>
    <el-upload ref="uploadRef" class="upload-demo" :action="uploadExcelUrl" :on-success="handleUploadSuccess"
      :on-error="handleUploadError" :headers="headers" :before-upload="handleBeforeUpload" :limit="1"
      :auto-upload="false">
      <template #trigger>
...
function handleUpdate(row) {
  ...
  getSku(_skuId).then(response => {
    ...
    form.value.price /= 100;  // 将价格单位从分转换为元
    open.value = true;
    title.value = "修改商品管理";
  });
}
...
function handleBeforeUpload(file) {
  ...
  proxy.$modal.loading("正在上传Excxel，请稍候...");
}
